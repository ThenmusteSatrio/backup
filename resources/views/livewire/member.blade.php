...
@push('scripts')
    ...
    <script>
        ...
        function deleteItem(nik) {
            Livewire.dispatch("delete", {
                nik: nik
            });
        }

        function getUpdateData(nik) {
...
@endpush
<x-panel>
    <div class="w-full h-full" x-data="{ modalMember: @entangle('modalMember') }">
        <div>
            @include('components.add-member-form')
        </div>
        @if (session('error'))
                        ...
                        <tr>
                            ...
                            <td class="">
                                <div class="flex items-center space-x-2">
                                    <button onclick="deleteItem({{ $item->nik }})"
                                    ...
                                    <button onclick="getUpdateData({{ $item->nik }})"
