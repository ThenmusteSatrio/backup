...
class Home extends Component
{
    ...
    {
        $carsByBrand = Car::where('brand', 'like', '%' . $this->inputSearch . '%')->get();
        $carsByType = Car::where('type', 'like', '%' . $this->inputSearch . '%')->get();
        if ($carsByBrand->count() > 0) {
            $this->cars = $carsByBrand;
        } elseif ($carsByType->count() > 0) {
            $this->cars = $carsByType;
        }
    }
